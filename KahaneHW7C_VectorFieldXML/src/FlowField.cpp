...
//NEW WAY NUMBER 1
//Sets all the forces in the field based ____NOT SURE WHAT TO CALL THIS?
void FlowField::sinYsinX(){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set(ofVec2f(sin(y),sin(x)));
        }
    }
}

//NEW WAY NUMBER 2
//Sets all the forces in the field based on ____________________________
void FlowField::negYposX(){
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set(ofVec2f( -y,x ));
            
            //            flowList[y][x].set(ofVec2f( 2*x*cos(y)-y*cos(x),(-x*-x)*sin(y)-sin(x)     ));
        }
    }
}

//NEW WAY NUMBER 3
//Sets all the forces in the field based on sinXcosY
